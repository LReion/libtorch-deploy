...
string test(const map<string, float> &m) // Note const keyword here 
{
  ...
  Document::AllocatorType &allocator = document.GetAllocator(); // Get a reference to the allocator 
...
// and uses them to run the program
  ...
  auto model = torch::jit::load(model_path); // Load the model from the given path 
  ...
  cv::resize(img, img, cv::Size(448, 448), cv::INTER_LINEAR); // Resize the image to 448 x 448 

  // Convert image data to a torch tensor
  torch::Tensor input_tensor = 
      torch::from_blob(img.data, {1, img.rows, img.cols, 3}, torch::kByte); 
  input_tensor = input_tensor.permute({0, 3, 1, 2}); // Permute the dimensions of the tensor  
...
vector v(output.data_ptr(), output.data_ptr() + output.numel());
...
target_map["predict_class"] = (float)(max_element(v.begin(), v.end()) - v.begin());
